...
function ViewUser() {
  const store = useStore<TCountStore>(countStore, ["user"]);

  return <div> <p>{store.user.name}</p> </div>;
}

function ViewUser2() {
  const store = useStore<TCountStore>(userStore, ["name"]);
